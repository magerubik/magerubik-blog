 ...
 */
namespace Magerubik\Blog\Model\ResourceModel;
/**
...
class Post extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $this->_init('magerubik_blog_post', 'post_id');
    }
    /**
    ...
    protected function _beforeDelete(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $this->getConnection()->delete($this->getTable('magerubik_blog_post_store'), $condition);
        $this->getConnection()->delete($this->getTable('magerubik_blog_post_category'), $condition);
        $this->getConnection()->delete($this->getTable('magerubik_blog_post_relatedproduct'), $condition);
        $this->getConnection()->delete($this->getTable('magerubik_blog_post_relatedpost'), $condition);
        $this->getConnection()->delete($this->getTable('magerubik_blog_post_relatedpost'),
    ...
    protected function _afterSave(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $this->_updateLinks($object, $newIds, $oldIds, 'magerubik_blog_post_store', 'store_id');
        $newIds = (array)$object->getCategories();
        if (is_array($newIds)) {
            $oldIds = $this->lookupCategoryIds($object->getId());
            $this->_updateLinks($object, $newIds, $oldIds, 'magerubik_blog_post_category', 'category_id');
        }
        $newIds = $object->getRelatedPostIds()== '' ? [] : $newIds = explode(',', $object->getRelatedPostIds());
        $oldIds = $this->lookupRelatedPostIds($object->getId());
        $this->_updateLinks($object, $newIds, $oldIds, 'magerubik_blog_post_relatedpost', 'related_id');
        $newIds = $object->getRelatedProductIds() == '' ? [] : $newIds = explode(',', $object->getRelatedProductIds());
        $oldIds = $this->lookupRelatedProductIds($object->getId());
        $this->_updateLinks($object, $newIds, $oldIds, 'magerubik_blog_post_relatedproduct', 'related_id');
    ...
    {
        $select = $this->getConnection()->select()->from(['cp' => $this->getMainTable()])->join(['cps' => $this->getTable('magerubik_blog_post_store')],'cp.post_id = cps.post_id',[])->where('cp.identifier = ?',$identifier)->where('cps.store_id IN (?)',$store);
        if (!is_null($isActive)) {
    ...
    {
        return $this->_lookupIds($postId, 'magerubik_blog_post_store', 'store_id');
    }
    /**
    ...
    {
        return $this->_lookupIds($postId, 'magerubik_blog_post_category', 'category_id');
    }
    /**
    ...
    {
        return $this->_lookupIds($postId, 'magerubik_blog_post_relatedpost', 'related_id');
    }
    /**
    ...
    {
        return $this->_lookupIds($postId, 'magerubik_blog_post_relatedproduct', 'related_id');
    }
    /**
